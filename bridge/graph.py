...
def _pending_action_parts(pending: list) -> tuple[str, str]:
    """(tool_name, hint) for the first pending tool call — the structured action the
    stall line is rendered from. ``hint`` is the best 'what' arg (a ticker, a query,
    a place…). Returns ("", "") when there's no usable pending call. Best-effort."""
    from bridge import server as S
    for tc in (pending or [])[:1]:
...
# Generalized stall renderer. The "thinking out loud" filler is DERIVED from the
# structured action (tool + hint), never free-generated by the small model — so it
# can't drift into the wrong domain (the "getting Tesla stock price data now" while
# finding a nocturne bug: the old LLM prompt's few-shots were all stock lines, so
# the 3B parroted them). Per-tool natural phrasings cover the common tools; a
# desk-family heuristic + a hint/vague generic fallback cover every other tool; a
# fixed safe line covers the COLD case (fired before a tool is even picked). Adding
# a tool needs no change here — it gets a sensible generic phrase automatically.
_COLD_STALL_LINES = ("one sec", "on it", "let me pull that up", "gimme a sec", "looking into it")

_STALL_PHRASES = {
    "web_search":       lambda h: ([f"looking up {h}", f"searching for {h}"] if h else ["looking that up", "let me search that"]),
    "get_stock_data":   lambda h: ([f"getting {h}'s price", f"pulling up {h}", f"checking on {h}"] if h else ["getting that price", "pulling up the quote"]),
    "get_news":         lambda h: ([f"pulling the latest on {h}", f"checking the news on {h}"] if h else ["checking the latest news", "pulling up headlines"]),
    "get_weather":      lambda h: ([f"checking the weather in {h}", f"getting the forecast for {h}"] if h else ["checking the weather", "getting the forecast"]),
    "calculate":        lambda h: ["crunching that", "doing the math", "working that out"],
    "video_player":     lambda h: ["finding that", "queuing that up", "pulling up a video", "finding a track"],
    "show_slides":      lambda h: ["putting that on screen", "building the slides"],
    "show_card":        lambda h: ["putting that up", "getting that for you"],
    "show_weather":     lambda h: ["pulling up the weather"],
    "show_diary":       lambda h: ["opening your diary"],
    "recall_diary":     lambda h: ["checking the diary", "flipping back through"],
    "recall_news":      lambda h: ["digging that up"],
    "schedule_cron":    lambda h: ["setting that up", "adding that to your schedule"],
    "set_display_name": lambda h: ["got it"],
}

_DESK_HINT_WORDS = ("pnl", "position", "trading", "balance", "risk", "trade",
                    "portfolio", "agent", "catalyst", "dossier", "briefing")


def _generic_stall_phrases(name: str, hint: str) -> list[str]:
    """Fallback phrasings for any tool not in _STALL_PHRASES: desk-flavored for the
    portfolio/desk family, otherwise name the hint if we have one, else stay vague."""
    n = name.lower()
    if any(k in n for k in _DESK_HINT_WORDS):
        return ([f"getting {hint} from the desk", "checking the desk"] if hint
                else ["pulling the desk numbers", "checking the desk"])
    if hint:
        return [f"looking up {hint}", "getting that for you"]
    return list(_COLD_STALL_LINES)
...
def _stall_phrase(name: str, hint: str, seed: int = 0) -> str:
    ...
    if not name:
        ...
        fn = _STALL_PHRASES.get(name)
        variants = (fn(hint) if fn else None) or _generic_stall_phrases(name, hint)
