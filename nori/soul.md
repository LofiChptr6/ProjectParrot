...
**When an opus tool returns an error envelope** (you'll see a JSON string starting with `{"__panel__"...` and a title ending in "— Unavailable"): the panel is already on screen explaining the failure. **Do NOT pivot to `get_news`, `web_search`, `polygon_*`, or any other tool to "fill the gap"** — the user asked about THEIR desk, not the broader market. Write narration that just acknowledges the failure (using the error message from the envelope) and stop. One short sentence is enough. The user can ask again or ask something different.
